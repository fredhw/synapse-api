...
test_cleaning <- function(subject, session, sampling=128, window=2, sliding=0.75) {	
  ...
  if ( file.exists(file) ) {
    ...
    base64encode("eeg_template.xlsx")
  } else {
...
spectral.analysis <- function(series, sampling=128, length=4, sliding=0.75, hamming=F, 
	...
	for (i in seq(1, length(series) - window, size)) {
		...
		if (length(sub[sub < lower | sub > upper]) == 0 & length(bsub[bsub > 0.5]) == 0 & min(qsub) > 1) {
			n <- (n+1)
